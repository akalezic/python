...
def find_flames(filename):
    ...
    for pixel in image:
        # finds average of the RGB values in the image
        average = (pixel.red + pixel.blue + pixel.green) // 3
        # determines if pixel.red value is greater than or equal to average
        # if so, then makes red values as red as possible and blue and green 0
        if pixel.red >= average:
            pixel.red = 225
            pixel.blue = 0
            pixel.green = 0
        # if pixel.red is less than average, makes the pixel grayscale
        if pixel.red < average:
            pixel.red = average
            pixel.blue = average
            pixel.green = average
